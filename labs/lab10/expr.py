...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        try:
            ...
            raise NameError('Exeption raised!')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        >>> CallExpr(Name('add'), [Literal(3), Name('a')]).eval(new_env)
        Number(4)
        ...
        """
        "*** YOUR CODE HERE ***"
        f = Name.eval(self.operator, env)
        o = [x.eval(env) for x in self.operands]
        return f.apply(o)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        for i in range(len(arguments)):
            e[self.parameters[i]] = arguments[i]
