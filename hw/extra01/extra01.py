...
def letters(tree):
    ...
    def helper(tree):
        for branch in tree:
            if type(branch) != int:
                if is_huffman_leaf(branch):
                    lst.append(letter(branch))
                else:
                    helper(branch)
    helper(tree)
    return lst
...
def decode_one(tree, code):
    ...
    """
    "*** YOUR CODE HERE ***"
    if type(letter(tree)) == str:
        return letter(tree)
    for i in code:
        new_tree = branches(tree)[i]
        code.remove(i)
        return decode_one(new_tree, code)
...
def encodings(tree):
    ...
    "*** YOUR CODE HERE ***"
    enc = {}
    def transverse(tree, lst = []):
        if is_huffman_leaf(tree):
            enc[letter(tree)] = lst
        else:
            transverse(branches(tree)[0], lst + [0])
            transverse(branches(tree)[1], lst + [1])
    transverse(tree)
    return enc
...
def add_fns(f_and_df, g_and_dg):
    ...
    "*** YOUR CODE HERE ***"
    h_and_dh = lambda x, derive: f_and_df(x, derive) + g_and_dg(x, derive)
    return h_and_dh
def mul_fns(f_and_df, g_and_dg):
    """Return the product of two polynomials."""
    "*** YOUR CODE HERE ***"
    h_and_dh = lambda x, derive: f_and_df(x, True) * g_and_dg(x, False) + f_and_df(x, False) * g_and_dg(x, True) if derive else f_and_df(x, False) * g_and_dg(x, False)
    return h_and_dh
def poly_zero(f_and_df):
    ...
    """
    "*** YOUR CODE HERE ***"
    f = lambda x: f_and_df(x, False)
    df = lambda x: f_and_df(x, True)
